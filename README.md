...
- [ ] Compositor (GUI land — the goal before circling back to threads/SMP)
  ...
  - [ ] Redraw / vsync trigger off the existing timer -- today's full-repaint-per-mouse-event is a stopgap; window content only updates when the mouse happens to move
  - [ ] Damage rects (only re-blit the region a client actually changed, instead of a full-frame push + full-screen repaint)
  - [ ] Input routing (hit-testing, focus) -- no client receives input yet, only the compositor's own cursor tracking
  - [x] Cursor rendering — see Compositor MVP above (8x8 bitmap sprite, full repaint each mouse event, PS/2 y-up negated to screen y-down)
